    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "complete":
          ...
          break;
        case "confirmation":
          setConfirmation(msg.confirmation);
          setCandidateDoctors([])
          break;
...
}, [user?.id, activeContext]); // ✅ ADDED activeContext to dependency array
...
useEffect(() => {
  ...
  const fetchConfirmation = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/confirmation/${user.id}`);
      console.log(response);
      
      if (!response.ok) {
        console.error(
          `Failed to fetch confirmation: ${response.status}`
        );
        return;
      }
